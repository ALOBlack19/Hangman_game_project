...
def category_menu():
    ...
    while True:
        ...
        if user_category in category_list:
            print(f"You choose: {user_category.capitalize()}\n")
    ...
    while True:
        try:
            ...
            if dif_level in [1, 2, 3]:
                attempts = {1: 8, # Making a dictionary which transform each difficult level in a key, and relate it for the respective number of attempts.
                            2: 6,
                            3: 3}[dif_level]
...
def game(win_word, attempts):
    ...
    guessed_letters = set() #  A different kind of list to automatically removing duplicates.
    ...
    while attempts_left > 0 and "_" in hidden_word: # While the user still has attempts and at the same time in hidden_word there is still "_", the game will keep running.
        print("\nCurrent word:", " ".join(hidden_word)) # Adding spaces between each letter, and join all list values in a word again.
        print(f"Attempts left: {attempts_left}")
        if guessed_letters: # If the user doesn't input and the set is still empty. # if guessed_letters: -> It give the condition to when guessed_letters is empty
            print(f"Guessed letters: {", ".join(guessed_letters)}")
        else:
             print(f"Guessed letters: {"None"}")
        ...
        guessed_letters.add(guess) # .add works to insert new data in a set()

        if guess in win_word:
            print("Good guess!")
            for i, letter in enumerate(win_word): # Enumerate combined each letter and it's respective index. "i" being the index and "letter", each letter of the win_word.
                if letter == guess:
                    hidden_word[i] = guess # Here is where occur the substitution of the "_" for the input letter guesses.
        else:
            print("Wrong guess!")
            attempts_left -= 1 # Subtract one every time the user guess wrongly.
